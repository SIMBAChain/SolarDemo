<template>
  <div style="overflow-x: hidden;">
    <div v-if="getUser().title == 'Prosumer' || getUser().title == 'Consumer'">
      <div class="md-layout md-gutter md-alignment-left">
              ...
              <md-card-header>
                <md-card-header-text>
                  <div class="md-subheading">
                    <md-radio v-if="getUser().title != 'Consumer'" v-model="isRunning" value="sell" v-on:change="changeStatus">Sell Extra Energy</md-radio>
                    <br>
                    <md-radio v-model="isRunning" value="buy" v-on:change="changeStatus">Buy Energy</md-radio>
                    <br>
                    <md-radio v-model="isRunning" value="off" v-on:change="changeStatus">Turn Off</md-radio>
                  </div>
        ...
        <div class="md-layout-item md-xlarge-size-20 md-large-size-20 md-medium-size-30 md-small-size-100 md-xsmall-size-100">
          <md-card class="my-card" md-with-hover>
            <md-ripple>
              ...
              <md-card-content>
                <div v-if="!loadingBar">
                  <div class="md-caption statistics" v-if="getUser().title != 'Consumer'">Total Sold Out: <strong style="color:#64dd17;">{{outReading}}</strong></div>
                  <div class="md-caption statistics">Total Buy In: <strong style="color:#d32f2f;">{{inReading}}</strong></div>
                  <div class="md-caption statistics" v-if="getUser().title != 'Consumer'">Available Credit: {{availableCredit}}</div>
        ...
        <div class="md-layout-item md-xlarge-size-60 md-large-size-60 md-medium-size-80 md-small-size-100 md-xsmall-size-100">
          <md-table class="table" v-model="searched" md-sort="name" md-sort-order="asc" md-fixed-header>
            ...
            <md-table-row v-if="getUser().title == 'Prosumer'" slot="md-table-row" v-for="(se, i) in searched" :key="i">
            ...
            <md-table-row v-if="getUser().title == 'Consumer'" slot="md-table-row" v-for="(se, i) in searched" :key="i">
              <md-table-cell md-label="No." md-numeric>{{ i + 1 }}</md-table-cell>
              <md-table-cell md-label="Type" md-sort-by="payload.inputs.txnType">{{ se.payload.inputs.txnType }}</md-table-cell>
              <md-table-cell md-label="Amount" md-sort-by="payload.inputs.txnAmount">{{ se.payload.inputs.txnAmount }}</md-table-cell>
      ...
      <div class="md-layout-item md-xlarge-size-60 md-large-size-60 md-medium-size-80 md-small-size-100 md-xsmall-size-100">
        <md-table class="table" v-model="searched" md-sort="name" md-sort-order="asc" md-fixed-header>
          ...
          <md-table-row slot="md-table-row" v-for="(se, i) in searched" :key="i" @click="queryTxn(se.id)">
            <md-table-cell md-label="No." md-numeric>{{ i + 1 }}</md-table-cell>
            <md-table-cell md-label="User" md-sort-by="payload.inputs.userName">{{ se.payload.inputs.userName }}</md-table-cell>
            <md-table-cell md-label="Role" md-sort-by="payload.inputs.userRole">{{ se.payload.inputs.userRole }}</md-table-cell>
            <md-table-cell md-label="Type" md-sort-by="payload.inputs.txnType">{{ se.payload.inputs.txnType }}</md-table-cell>
            <md-table-cell md-label="Amount" md-sort-by="payload.inputs.txnAmount">{{ se.payload.inputs.txnAmount }}</md-table-cell>
            <md-table-cell md-label="Timestamp" md-sort-by="timestamp">{{ (new Date(se.timestamp)).toLocaleString() }}</md-table-cell>
          </md-table-row>
            ...
            <md-tab md-label="Basic">
              ...
              <div>
                <div class="md-layout md-gutter">
                  ...
                  <div v-if="checkedTxn.userRole == 'Prosumer'" class="md-layout-item md-size-40">
                    <md-field>
              ...
              <div v-if="checkedTxn.userRole == 'Prosumer'">
                <div class="md-layout md-gutter">
                  <div class="md-layout-item md-size-40">
                    <md-field>
...
export default {
  ...
  mounted () {
    ...
    if (this.getUser().title !== 'Utility Representative') {
      this.getUserTxns()
    } else {
      this.getAllTxns()
      let self = this
